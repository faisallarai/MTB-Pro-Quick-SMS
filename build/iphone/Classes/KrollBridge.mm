...
@implementation MTBProQuickSMSObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_mtbproquicksms);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any MTBProQuickSMS patches in JS before a context is loaded... nice for 
...
{
	// create MTBProQuickSMS global object
	NSString *basePath = (url==nil) ? [[NSBundle mainBundle] resourcePath] : [[url path] stringByDeletingLastPathComponent];
	_mtbproquicksms = [[MTBProQuickSMSObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_mtbproquicksms];
	
	NSString *_mtbproquicksmsNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithUTF8CString([_mtbproquicksmsNS UTF8String]);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_mtbproquicksms valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.mtbproquicksms.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
